export function estimateRangeMiles(usableBatteryKwh: number, kwhPer100mi: number): number | null {
  if (!Number.isFinite(usableBatteryKwh) || usableBatteryKwh < 0 || !Number.isFinite(kwhPer100mi) || kwhPer100mi <= 0) return null;
...
export function estimateCostPerMile(kwhPer100mi: number, pricePerKwh: number): number | null {
  if (!Number.isFinite(kwhPer100mi) || kwhPer100mi < 0 || !Number.isFinite(pricePerKwh) || pricePerKwh < 0) return null;
...
export function estimateCostPerCharge(usableBatteryKwh: number, pricePerKwh: number): number | null {
  if (!Number.isFinite(usableBatteryKwh) || usableBatteryKwh < 0 || !Number.isFinite(pricePerKwh) || pricePerKwh < 0) return null;
  return usableBatteryKwh * pricePerKwh;
}

export function estimateGasCostPerMile(mpg: number, pricePerGallon: number): number | null {
  if (!Number.isFinite(mpg) || mpg <= 0 || !Number.isFinite(pricePerGallon) || pricePerGallon < 0) return null;
